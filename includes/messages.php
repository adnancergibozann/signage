...
{
    $pdo = get_pdo();
    $stmt = $pdo->query('SELECT id, title, body, text_color, background_color, speed FROM ticker_messages ORDER BY position ASC, id ASC');
...
{
    $pdo = get_pdo();
    $stmt = $pdo->prepare('INSERT INTO ticker_messages (title, body, text_color, background_color, speed, position) VALUES (:title, :body, :text_color, :background_color, :speed, :position)');
    $stmt->execute([
        'title' => $data['title'],
        'body' => $data['body'],
